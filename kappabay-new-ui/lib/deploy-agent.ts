...
export async function deployAgent(
  ...
  tx.moveCall({
    ...
    arguments: [
      tx.pure(
        bcs
          .vector(bcs.u8())
          .serialize(Array.from(Buffer.from(serializeAgentConfig(agentConfig))))
      ),
      coin,
  ...
  if (txResult.objectChanges && Array.isArray(txResult.objectChanges)) {
    for (const c of txResult.objectChanges) {
      if (c.type === "created") {
        if (c.objectType.includes("Agent")) agentObjectId = c.objectId;
        else if (c.objectType.includes("AgentCap")) agentCapId = c.objectId;
