...
import { useCurrentWallet } from "@mysten/dapp-kit"; // adjust import if needed
  ...
  useEffect(() => {
    async function fetchAgents() {
      ...
      const res = await fetch(
        `/api/agents?ownerWallet=${encodeURIComponent(currentWallet.address)}`
      );
      if (res.ok) {
  ...
  return (
    <div className="container mx-auto p-4">
      <header className="flex items-center justify-between mb-8">
        ...
        <Button onClick={() => {/* Navigate to deploy new agent page */}}>
          Deploy New Agent
        </Button>
      </header>
